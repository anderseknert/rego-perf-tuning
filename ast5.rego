...
_find_vars(path, value) := _find_assign_vars(path, value) if {
...
_find_vars(path, value) := _find_some_in_decl_vars(path, value) if {
	path[count(path) - 1] == "symbols"

	value[0].type == "call"
}

_find_vars(path, value) := _find_some_decl_vars(path, value) if {
	path[count(path) - 1] == "symbols"

	value[0].type != "call"
}

_find_vars(path, value) := _find_every_vars(path, value) if {
	path[count(path) - 1] == "terms"
	value.domain
}
